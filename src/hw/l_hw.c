...
{
	uint32_t port = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
	uint32_t clockspeed = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
...
static int l_hw_spi_transfer(lua_State* L)
{
	...
	uint32_t repeat = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 5));
	int8_t chip_select = (int8_t) lua_tonumber(L, ARG1 + 6);
	uint32_t cs_delay_us = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 7));
	// Begin the transfer
...
{
	uint32_t port = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
	uint32_t address = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
...
{
	uint32_t port = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
	uint32_t baudrate = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
...
{
	uint32_t pin = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
...
{
	uint32_t pin = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
	uint32_t level = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
...
static int l_sct_read_pulse(lua_State* L)
{
	...
	uint32_t timeout = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
...
static int l_hw_pwm_port_period(lua_State *L) {
	uint32_t period = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1));
...
static int l_hw_pwm_pin_pulsewidth(lua_State* L)
{
	...
	uint32_t pulsewidth = DOUBLE_TO_UINT32(lua_tointeger(L, ARG1 + 1));
