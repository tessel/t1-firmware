...
#define CAN_IF_Read(LPCx, reg, IFsel) (##LPCx##->##IFsel##_##reg)
#define CAN_IF_Write(LPCx, reg, IFsel, val) (##LPCx##->##IFsel##_##reg=val)
#else
#define CAN_IF_Read(LPCx, reg, IFsel) (LPCx->IFsel##_##reg)
#define CAN_IF_Write(LPCx, reg, IFsel, val) (LPCx->IFsel ## _ ## reg = val)
#endif

/* number of C_CAN IP in LPC18xx */
#define MAX_CAN_IP	2
...
MSG_CB TX_cb[MAX_CAN_IP], RX_cb[MAX_CAN_IP];

//message_object can_buff[CAN_MSG_OBJ_MAX][MAX_CAN_IP];
message_object recv_buff[MAX_CAN_IP];
...
void CAN_IF_readBuf(LPC_C_CANn_Type* LPC_C_CANx, uint32_t IFsel,uint32_t msg){
	if(IFsel == IF1){
	LPC_C_CANx->IF1_CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
	LPC_C_CANx->IF1_CMDREQ=msg;
	while (LPC_C_CANx->IF1_CMDREQ & IFCREQ_BUSY );
	}else{
	  LPC_C_CANx->IF2_CMDMSK_W=RD|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
	  LPC_C_CANx->IF2_CMDREQ=msg;
	  while (LPC_C_CANx->IF2_CMDREQ & IFCREQ_BUSY );
  }

}

/* Macro for reading specific RAM message object to IF */
void CAN_IF_writeBuf(LPC_C_CANn_Type* LPC_C_CANx, uint32_t IFsel,uint32_t msg){
	if(IFsel == IF1){
  LPC_C_CANx->IF1_CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
  LPC_C_CANx->IF1_CMDREQ=msg;
  while (LPC_C_CANx->IF1_CMDREQ & IFCREQ_BUSY );
	}else{
		  LPC_C_CANx->IF2_CMDMSK_W=WR|MASK|ARB|CTRL|CLRINTPND|DATAA|DATAB;
		  LPC_C_CANx->IF2_CMDREQ=msg;
		  while (LPC_C_CANx->IF2_CMDREQ & IFCREQ_BUSY );
	}
}
 ...
 * @return 		None
 **********************************************************************/
void CAN_TxInt_MessageProcess(LPC_C_CANn_Type* LPC_C_CANx, uint8_t msg_no )
{
	/* Clear interrupt pending bit */
	CAN_IF_Write(LPC_C_CANx, MCTRL, IF2, UMSK|RXIE|EOB|CAN_DLC_MAX);
	/* Save changes to message RAM */
...
void CAN_IRQHandler(void)
{
		...
		else
		{
			if ( (canstat & STAT_LEC) == 0 ) 	/* NO ERROR */
			{
				...
				if((msg_no >= 1 ) && (msg_no <= 16))
				{
					...
					CAN_RxInt_MessageProcess(LPC_C_CAN0, msg_no);
					RX_cb[0](msg_no);
				}
				else
				{
				 	LPC_C_CAN0->STAT &= ~STAT_TXOK;
					CAN_TxInt_MessageProcess(LPC_C_CAN0, msg_no);
					TX_cb[0](msg_no);
				}
			}
		}
	...
	/* C_CAN 1 interrupt */
	while ( (can_int = LPC_C_CAN1->INT) != 0 )	/* While interrupt is pending */
	{
		canstat = LPC_C_CAN1->STAT;				/* Read CAN status register */
		...
		if ( can_int & CAN_STATUS_INTERRUPT )
		{
			...
			/* Clear all warning/error states except RXOK/TXOK */
			LPC_C_CAN1->STAT &= STAT_RXOK|STAT_TXOK;
		}
		else
		{
			if ( (canstat & STAT_LEC) == 0 ) 	/* NO ERROR */
			{
				...
				if((msg_no >= 1 ) && (msg_no <= 16))
				{
					LPC_C_CAN1->STAT &= ~STAT_RXOK;
					/* Check if message number is correct by reading NEWDAT registers.
					 By reading out the NEWDAT bits, the CPU can check for which Message
					 Object the data portion was updated
					 Only first 16 message object used for receive : only use ND1 */
					if((1<<(msg_no-1)) != LPC_C_CAN1->ND1)
					{
						/* message object does not contain new data! */
						ND1ErrCnt++;
						break;
					}
					CAN_RxInt_MessageProcess(LPC_C_CAN1, msg_no);
					RX_cb[1](msg_no);
				}
				else
				{
				 	LPC_C_CAN1->STAT &= ~STAT_TXOK;
					CAN_TxInt_MessageProcess(LPC_C_CAN1, msg_no);
					TX_cb[1](msg_no);
				}
			}
		}
 ...
 * @return 		None
 **********************************************************************/
void CAN_Init(LPC_C_CANn_Type* LPC_C_CANx, uint32_t BitClk, CCAN_CLKDIV_Type ClkDiv , MSG_CB Tx_cb, MSG_CB Rx_cb)
{
	if(LPC_C_CANx == LPC_C_CAN0){
		RX_cb[0] = Rx_cb;
		TX_cb[0] = Tx_cb;
	...
	if ( !(LPC_C_CANx->CNTL & CTRL_INIT) )
	{
		/* If it's in normal operation already, stop it, reconfigure
		 everything first, then restart.  */
		LPC_C_CANx->CNTL |= CTRL_INIT;	/* Default state */
	}

	LPC_C_CANx->CLKDIV = ClkDiv;			/* Divider for CAN VPB3 clock */
	LPC_C_CANx->CNTL |= CTRL_CCE;		/* Start configuring bit timing */
	LPC_C_CANx->BT = BitClk;
	LPC_C_CANx->BRPE = 0x0000;
	LPC_C_CANx->CNTL &= ~CTRL_CCE;		/* Stop configuring bit timing */

	LPC_C_CANx->CNTL &= ~CTRL_INIT;		/* Initialization finished, normal operation now. */
	while ( LPC_C_CANx->CNTL & CTRL_INIT );

	/* By default, auto TX is enabled, enable all related interrupts */
	LPC_C_CANx->CNTL |= (CTRL_IE|CTRL_SIE|CTRL_EIE);
	return;
}
 ...
 * @return 		None
 **********************************************************************/
void CAN_Send(LPC_C_CANn_Type* LPC_C_CANx, uint8_t msg_no, uint32_t *msg_ptr )
{
	...
	CAN_IF_Write(LPC_C_CANx, MCTRL, IF1, UMSK|TXIE|TXRQ|EOB|RMTEN|(Length & DLC_MASK));
	CAN_IF_Write(LPC_C_CANx, DA1, IF1, *msg_ptr);			/* Lower two bytes of message pointer */
	CAN_IF_Write(LPC_C_CANx, DA2, IF1, (*msg_ptr++)>>16);	/* Upper two bytes of message pointer */
	CAN_IF_Write(LPC_C_CANx, DB1, IF1, *msg_ptr);			/* Lower two bytes of message pointer */
	CAN_IF_Write(LPC_C_CANx, DB2, IF1, (*msg_ptr)>>16);		/* Upper two bytes of message pointer */

	/* Configure arbitration */
	if(!(tx_id & (0x1<<30)))					/* bit 30 is 0, standard frame */
	{
		/* Mxtd: 0, Mdir: 1, Mask is 0x7FF */
		CAN_IF_Write(LPC_C_CANx, MSK2, IF1, MASK_MDIR | (ID_STD_MASK << 2));
		CAN_IF_Write(LPC_C_CANx, MSK1, IF1, 0x0000);

		/* MsgVal: 1, Mtd: 0, Dir: 1, ID = 0x200 */
		CAN_IF_Write(LPC_C_CANx, ARB1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, ARB2, IF1, ID_MVAL| ID_DIR | (tx_id << 2));
	}
	else										/* Extended frame */
	{
		/* Mxtd: 1, Mdir: 1, Mask is 0x7FF */
		CAN_IF_Write(LPC_C_CANx, MSK2, IF1, MASK_MXTD | MASK_MDIR | (ID_EXT_MASK >> 16));
		CAN_IF_Write(LPC_C_CANx, MSK1, IF1, ID_EXT_MASK & 0x0000FFFF);

		/* MsgVal: 1, Mtd: 1, Dir: 1, ID = 0x200000 */
		CAN_IF_Write(LPC_C_CANx, ARB1, IF1, tx_id & 0x0000FFFF);
		CAN_IF_Write(LPC_C_CANx, ARB2, IF1, ID_MVAL|ID_MTD | ID_DIR | (tx_id >> 16));
	}

  	/* Write changes to message RAM */
	CAN_IF_writeBuf(LPC_C_CANx, IF1, msg_no);
 ...
 * @return 		None
 **********************************************************************/
void CAN_Recv(LPC_C_CANn_Type* LPC_C_CANx, uint8_t msg_no, uint32_t *msg_ptr, Bool RemoteEnable)
{
	...
	if(!(rx_id & (0x1<<30))){ /* standard frame */
 		...
 		/* Mxtd: 0, Mdir: 0, Mask is 0x7FF */
	  	CAN_IF_Write(LPC_C_CANx, MSK1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, MSK2, IF1, ID_STD_MASK << 2);
		/* MsgVal: 1, Mtd: 0, Dir: 0 */
		CAN_IF_Write(LPC_C_CANx, ARB1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, MCTRL, IF1, rmten|UMSK|RXIE|EOB|CAN_DLC_MAX);
		CAN_IF_Write(LPC_C_CANx, DA1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DA2, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DB1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DB2, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, ARB2, IF1, ID_MVAL | ((rx_id) << 2));
		...
		CAN_IF_Write(LPC_C_CANx, MSK1, IF1, ID_EXT_MASK & 0xFFFF);
		CAN_IF_Write(LPC_C_CANx, MSK2, IF1, MASK_MXTD | (ID_EXT_MASK >> 16));
		/* MsgVal: 1, Mtd: 1, Dir: 0 */
		CAN_IF_Write(LPC_C_CANx, ARB1, IF1, (rx_id) & 0xFFFF);
		CAN_IF_Write(LPC_C_CANx, MCTRL, IF1, rmten|UMSK|RXIE|EOB|CAN_DLC_MAX);
		CAN_IF_Write(LPC_C_CANx, DA1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DA2, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DB1, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, DB2, IF1, 0x0000);
		CAN_IF_Write(LPC_C_CANx, ARB2, IF1, ID_MVAL | ID_MTD | ((rx_id) >> 16));
		/* Transfer data to message RAM */
		CAN_IF_writeBuf(LPC_C_CANx, IF1, msg_no);
	}
	return;
 ...
 * @return 		None
 **********************************************************************/
void CAN_ReadMsg(LPC_C_CANn_Type* LPC_C_CANx, uint32_t msg_no, message_object* buff){
	int i;
	if(LPC_C_CANx == LPC_C_CAN0){
		buff->id = recv_buff[0].id;
		buff->dlc = recv_buff[0].dlc;
		if(recv_buff[0].dlc>CAN_DLC_MAX) recv_buff[0].dlc = CAN_DLC_MAX;
		for(i=0;i<recv_buff[0].dlc;i++)
			buff->data[i] = recv_buff[0].data[i];
	}else if(LPC_C_CANx == LPC_C_CAN1){
		buff->id = recv_buff[1].id;
		buff->dlc = recv_buff[1].dlc;
		if(recv_buff[1].dlc>CAN_DLC_MAX) recv_buff[1].dlc = CAN_DLC_MAX;
		for(i=0;i<recv_buff[1].dlc;i++)
			buff->data[i] = recv_buff[1].data[i];
	}
}
