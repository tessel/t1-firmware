...
//	char * lcd_string = logo;
	...
	scu_pinmux(0xF,1,(MD_PUP | MD_EHS),FUNC2);//SSP0_SSEL
 	scu_pinmux(0xD,0,MD_PUP,FUNC4);	// PD.0 connected to GPIO = LCD_RST
 	scu_pinmux(0x2,5,MD_PUP,FUNC4);	// P2.5 connected to GPIO = LCD_A0
	scu_pinmux(0xF,0,(MD_PUP | MD_EHS),FUNC0);//SSP0_CLK
	scu_pinmux(0xF,3,(MD_PUP | MD_EHS),FUNC2);//SSP0_MISO

	//Set the directions
	GPIO_SetDir(0x6,(1<<10), 1);
	GPIO_SetDir(0x6,(1<<11), 1);

	GPIO_SetDir(0x6,(1<<14), 1);
	GPIO_SetDir(0x5,(1<<5), 1);

	GPIO_ClearValue(6,(1<<10));
	GPIO_ClearValue(6,(1<<11));
	delay(100);
	//Reset lcd 
	GPIO_SetValue(6,(1<<14));
	delay(100);
	GPIO_ClearValue(6,(1<<14));
	delay(100);
	GPIO_SetValue(6,(1<<14));
	delay(100);
	GPIO_SetValue(5,(1<<5));  //set A0
	delay(100);
